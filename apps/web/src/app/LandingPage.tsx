...
const FEATURES = [
  { icon: DollarSign, title: 'Name Your Price', description: 'Set your own fare. No surge pricing, no guessing. Drivers see your offer and decide.', iconBg: 'bg-primary/10', iconColor: 'text-primary' },
  { icon: Shield, title: 'Safety First', description: 'All drivers verified, background-checked, and rated. Your safety is our priority.', iconBg: 'bg-secondary/10', iconColor: 'text-secondary' },
  { icon: Zap, title: 'Lightning Fast', description: 'Matches in under 30 seconds. Real-time tracking from pickup to drop-off.', iconBg: 'bg-[#F59E0B]/10', iconColor: 'text-[#F59E0B]' },
  { icon: Clock, title: 'Schedule Rides', description: 'Plan ahead. Schedule rides up to 7 days in advance at your preferred price.', iconBg: 'bg-[#EF4444]/10', iconColor: 'text-[#EF4444]' },
];
...
export function LandingPage({ appName, tagline, logoText }: LandingPageProps) {
    ...
    <div className="min-h-screen bg-background text-white overflow-x-hidden">
      ...
      <section ref={heroRef} className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <motion.div style={{ opacity: heroOpacity, y: heroY }} className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 text-center pt-24 pb-20">
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }} className="inline-flex items-center gap-2 bg-primary/10 border border-primary/30 rounded-full px-4 py-1.5 text-sm text-primary font-medium mb-8">
          ...
          <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }} className="text-xl sm:text-2xl text-white/50 max-w-2xl mx-auto mb-10 leading-relaxed">
            {tagline}. Propose your fare, drivers compete, you choose the best offer.
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0, y: 60, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.8, delay: 0.5 }} className="mt-16 max-w-sm mx-auto">
            <div className="card glass p-5 text-left rounded-3xl">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-full bg-gradient-cta flex items-center justify-center text-sm font-bold">JD</div>
                <div>
                ...
                <div className="ml-auto font-mono font-bold text-secondary text-xl">$12</div>
              </div>
              <div className="space-y-2 text-sm">
                <div className="flex items-start gap-2"><MapPin size={14} className="text-primary mt-0.5 flex-shrink-0" /><span className="text-white/70">123 Main St, Downtown</span></div>
                <div className="w-px h-4 ml-[7px] border-l border-dashed border-white/20" />
                <div className="flex items-start gap-2"><MapPin size={14} className="text-secondary mt-0.5 flex-shrink-0" /><span className="text-white/70">Airport Terminal B</span></div>
              </div>
              <div className="mt-4 flex items-center gap-2 text-xs text-white/40">
                <Clock size={12} /><span>Arriving in 3 min</span>
                <span className="ml-auto text-success font-medium">● Accepted</span>
              </div>
            </div>
          </motion.div>
        </motion.div>
      ...
      <section id="features" className="py-24 max-w-7xl mx-auto px-4 sm:px-6">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-100px' }} variants={containerVariants} className="text-center mb-16">
          <motion.div variants={itemVariants} className="text-primary text-sm font-semibold uppercase tracking-widest mb-4">Why {appName}</motion.div>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={containerVariants} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {FEATURES.map((feature) => (
            <motion.div key={feature.title} variants={itemVariants} whileHover={{ y: -4, scale: 1.01 }} className="card card-interactive p-6 rounded-2xl">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 ${feature.iconBg}`}>
      ...
      <section id="how-it-works" className="py-24 bg-surface">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={containerVariants} className="text-center mb-16">
            <motion.div variants={itemVariants} className="text-secondary text-sm font-semibold uppercase tracking-widest mb-4">Simple process</motion.div>
      ...
      <section className="py-24 max-w-7xl mx-auto px-4 sm:px-6">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={containerVariants} className="text-center mb-16">
          <motion.div variants={itemVariants} className="text-primary text-sm font-semibold uppercase tracking-widest mb-4">Testimonials</motion.div>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={containerVariants} className="grid md:grid-cols-3 gap-6">
          {TESTIMONIALS.map((t) => (
            <motion.div key={t.name} variants={itemVariants} whileHover={{ y: -4 }} className="card p-6 rounded-2xl">
              ...
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-gradient-cta flex items-center justify-center text-xs font-bold">{t.avatar}</div>
      ...
      <section className="py-24">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <motion.div
            ...
            viewport={{ once: true }}
            className="relative overflow-hidden rounded-3xl p-12 border border-primary/30"
            style={{ background: 'linear-gradient(135deg, rgb(var(--color-primary-rgb)/0.2), rgb(var(--color-secondary-rgb)/0.2))' }}
          >
      ...
      <footer className="border-t border-white/[0.06] py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg bg-gradient-cta flex items-center justify-center"><Car size={13} className="text-white" /></div>
            <span className="font-black text-lg">{appName}</span>
          </div>
          <div className="text-white/30 text-sm">© 2026 {appName}. All rights reserved.</div>
