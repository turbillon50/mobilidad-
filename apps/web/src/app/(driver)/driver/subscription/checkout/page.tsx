    ...
    <div className="min-h-screen bg-[#0A0A0F]">
      <div className="safe-top px-4 pt-4 pb-4 border-b border-[rgba(255,255,255,0.06)]">
        <div className="flex items-center gap-3">
          <Link href="/driver/subscription" className="text-[#8B8B9E] hover:text-white transition-colors"><ArrowLeft size={22} /></Link>
          <h1 className="font-black text-xl">Activar Plan</h1>
        </div>
      </div>

      <div className="px-4 py-6 space-y-5 max-w-md mx-auto">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="bg-gradient-to-br from-[rgba(108,99,255,0.2)] to-[rgba(0,212,170,0.1)] border border-[rgba(108,99,255,0.25)] rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            <div><div className="font-black text-lg">Plan Mensual</div><div className="text-xs text-[#8B8B9E]">Facturado mensualmente</div></div>
            <div className="text-right">
              <div className="text-3xl font-black font-mono bg-gradient-to-r from-[#6C63FF] to-[#00D4AA] bg-clip-text text-transparent">$299</div>
              <div className="text-xs text-[#8B8B9E]">MXN/mes</div>
            </div>
          </div>
          <div className="space-y-1.5">
            {features.map(f => (<div key={f} className="flex items-center gap-2 text-xs"><Check size={12} className="text-[#2ED573]" /><span className="text-[#8B8B9E]">{f}</span></div>))}
          </div>
        </motion.div>
        ...
        <motion.form initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }} onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-[#111118] border border-[rgba(255,255,255,0.06)] rounded-2xl overflow-hidden">
            <div className="px-4 py-3 border-b border-[rgba(255,255,255,0.04)]">
              <p className="text-xs font-semibold text-[#8B8B9E] uppercase tracking-wider flex items-center gap-2"><CreditCard size={12} /> Datos de la tarjeta</p>
            </div>
            ...
            <div className="px-4 py-3.5 border-t border-[rgba(255,255,255,0.04)]"><label className="text-xs text-[#4A4A5A] block mb-1.5">Nombre en la tarjeta</label><input type="text" placeholder="Como aparece en la tarjeta" value={name} onChange={e => setName(e.target.value)} className="w-full bg-transparent text-sm outline-none placeholder-[#2A2A3A]" /></div>
          </div>

          {error && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-sm text-[#FF4757] text-center">{error}</motion.p>}

          <button type="submit" disabled={loading} className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#6C63FF] to-[#00D4AA] font-bold flex items-center justify-center gap-2 disabled:opacity-50">
